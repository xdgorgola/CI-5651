...
def convex_hull(points: list[vector]) -> list[vector]:
    ...
    for i in range(1,len(points)):
        while cross_sign(vector_from_points(ch[-1], points[i]), vector_from_points(ch[-2], ch[-1])) > 0:
            nch.append(ch.pop())
        ch.append(points[i])
    return ch, nch
...
def onion_ch(points: list[vector]) -> int:
    ...
    plt.title("Capas de convex hull")
    plt.show()
    return layers
...
pointst = [(a,0) for a in arange(0.1, 10, 2.5)] + [(-a,0) for a in arange(0.1, 10, 2.5)] + [(0,a) for a in arange(0.1, 10, 2.5)] + [(0,-a) for a in arange(0.1, 10, 2.5)]
